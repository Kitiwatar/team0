...
class Genmod extends CI_Model{
  ...
  public function getLastTask($id){ 
    $sql = "SELECT tl_name FROM `pms_task` LEFT JOIN `pms_tasklist` ON pms_task.t_tl_id = pms_tasklist.tl_id WHERE pms_task.t_createdate <= NOW() AND t_p_id = ? AND t_status=1 ORDER BY pms_task.t_createdate DESC LIMIT 1"; 
